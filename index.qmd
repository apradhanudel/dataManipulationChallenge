---
title: "Ashwini's Data Manipulation Challenge"
...
# SSL Fix for macOS - Add this to bypass SSL certificate issues
import ssl
...
#### Briefly Give Answers to the Discussion Questions In This Section for Mental Model 1: Assign
> **Answer to Question 1**: Finding the dtype of actualShipDate using python code
```python
# Answer to Question 1: Finding the dtype of actualShipDate using python code
print("actualShipDate dtype:", shipments_df['actualShipDate'].dtype)
print("plannedShipDate dtype:", shipments_df['plannedShipDate'].dtype)

# check if they're the same type
print("Are both columns the same dtype?", 
      shipments_df['actualShipDate'].dtype == shipments_df['plannedShipDate'].dtype)
      
# Show sample values to understand the data
print("\nSample values:")
print("actualShipDate:", shipments_df['actualShipDate'].head(3).tolist())
print("plannedShipDate:", shipments_df['plannedShipDate'].head(3).tolist())
```

> **Answer to Question 2**: why is it important that both `actualShipDate` and `plannedShipDate` have the same data type for comparison?
Because if they don't have the same data type, you will get a TypeError when you try to compare them.
...
```
What's wrong with the `lateStatement` assignment and how would you fix it?
> **Answer to Question 3**: lambda function is not being used for the lateStatement assignment to reference the dataframe being used. Corrected code:
```{python}
# Corrected code:
shipments_with_lateness = (
    shipments_df
...
#### Briefly Give Answers to the Discussion Questions In This Section for Mental Model 2: Subset
> **Answer to Question 1**: Query vs Boolean Indexing
- The difference between using `.query('is_late == True')` and `[df['is_late'] == True]` is that `.query()` is a more readable way to query the dataframe and `[df['is_late'] == True]` is a more concise way to query the dataframe.
- The more readable approach is `.query('is_late == True')` because it is more readable and easier to understand.

> **Answer to Question 2**: Additional Row Querying
- Can you show an example of using a variable like `late_threshold` to query rows for shipments that are at least `late_threshold` days late, e.g. what if you wanted to query rows for shipments that are at least 5 days late?
```{python}
# Answer to Question 2: Additional Row Querying
late_shipments = shipments_with_lateness.query('days_late >= 5')
print(late_shipments.head())
...
#### Briefly Give Answers to the Discussion Questions In This Section for Mental Model 3: Drop

> **Answer to Question 1**: Drop vs Filter Strategies
- The difference between `.drop(columns=['quantity'])` and `.filter()` with a list of columns you want to keep is that `.drop(columns=['quantity'])` drops the column from the dataframe and `.filter()` keeps the columns in the dataframe.
- You would choose to drop columns if you don't need them for the analysis and you would choose to filter to keep specific columns if you need to keep the columns for the analysis.

> **Answer to Question 2**: Handling Missing Data
- If you use `.dropna()` without specifying `subset`, it will drop all rows with any missing values in any column.
- You might want to be selective about which columns to check for missing values if you know that some columns are more important than others.
...
- The difference between `ascending=False` and `ascending=True` in sorting is that `ascending=False` sorts the data in descending order and `ascending=True` sorts the data in ascending order.
- You would sort by multiple columns by using the `sort_values()` method and passing a list of columns to sort by.
Example code:
```{python}
# Example code:
sorted_by_lateness = clean_shipments.sort_values(['is_late', 'days_late'], ascending=[False, True])
print(sorted_by_lateness.head())
```
...
- The original index is lost when you sort the dataframe. This is problematic because it can cause issues with the dataframe being used in other code.
...
- The professor thinks we should use `how='left'` in most cases because it keeps all the rows from the left dataframe and only joins the rows from the right dataframe that have matching values in the key column.
- You can check if any shipments were lost during the merge by checking if there are any rows in the left dataframe that do not have a matching value in the key column of the right dataframe.

> **Answer to Question 2**: Key Column Matching
- If there are duplicate `partID` values in the `product_line_df`, it will cause the merge to fail.
- You can check if there are duplicate `partID` values in the `product_line_df` by using the `value_counts()` method.

Example code:
```{python}
# Example code:
product_line_df = product_line_df.drop_duplicates(subset=['partID'])
...
- `.groupby('prodCategory')` actually does the following:
    - It splits the data into groups based on the values in the `prodCategory` column.
    - It then applies the aggregation functions to each group.
- We need to use `.agg()` after grouping because it allows us to apply multiple aggregation functions to the data.

> **Answer to Question 2**: Multi-Level Grouping
- Grouping by `['shipID', 'prodCategory']` is different from grouping by `'prodCategory'` alone because it groups the data by both the `shipID` and `prodCategory` columns.
- This allows us to answer questions about the data at the shipment level as well as the product category level.
...
> **Answer to Question 1**: Business Question Analysis
- The comprehensive analysis answers the business question of "What is the percentage of shipments that have multiple product categories?"
- Grouping by `['shipID', 'prodCategory']` differs from grouping by just `'prodCategory'` because it groups the data by both the `shipID` and `prodCategory` columns and allows to handle duplicate `shipID` values due to a particular order having multiple partID's.
- ZappTech's management can gain insights from knowing the percentage of multi-category shipments to improve their service level. This can help them to identify shipments that have multiple product categories and to improve their service level for those shipments. This will also help them understand which product categories are causing the most issues and to improve their service level for those product categories.
...
Add code to create the visualization:
...
# Create a barplot showing late shipments by product category
plt.figure(figsize=(10, 5))

# Calculate the percentage of late shipments by category
late_by_category = (
    shipments_with_category
...
late_by_category.columns = ['total_shipments', 'late_shipments']
late_by_category['late_percentage'] = (late_by_category['late_shipments'] / late_by_category['total_shipments'] * 100).round(2)

# Create the barplot
sns.barplot(
    data=late_by_category.reset_index(),
    x='prodCategory',
    y='late_percentage',
    palette='viridis',
...
plt.title('Percentage of Late Shipments by Product Category', fontsize=14, fontweight='bold')
plt.xlabel('Product Category', fontsize=11)
plt.ylabel('Percentage of Late Shipments (%)', fontsize=11)
